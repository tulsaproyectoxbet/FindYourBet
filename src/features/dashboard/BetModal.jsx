...
export function BetModal({ open, onClose, form, setForm, onSubmit, user, preselectedChannelId }) {
  ...
  const handleImageFile = async (file) => {
    if (!file || !file.type.startsWith('image/')) { setUploadError(t('betModal.onlyImages')); return }
    setUploading(true)
    setUploadError('')
    try {
      ...
      if (error) { setUploadError(`${t('betModal.uploadError')}: ${error.message}`); return }
      const { data: urlData } = supabase.storage.from('channel-files').getPublicUrl(path)
      set('betImageUrl', urlData.publicUrl)
    } catch {
      setUploadError(t('betModal.uploadUnexpected'))
    } finally {
      setUploading(false)
    }
  }
  ...
  const handleCreateChannel = async () => {
    if (!createForm.name.trim()) { setCreateError(t('betModal.nameRequired')); return }
    setCreating(true)
    setCreateError('')
    ...
    if (error) { setCreateError(t('betModal.createChannelError')); setCreating(false); return }
      ...
      {myChannels.length === 0 && !showCreateChannel ? (
        ...
        <>
          ...
          <div style={{ display: 'flex', gap: '4px', background: 'var(--color-bg-soft)', border: '0.5px solid var(--color-border)', borderRadius: 'var(--radius-lg)', padding: '3px', marginBottom: '10px' }}>
            {[
              { id: 'public',  icon: 'globe', label: t('betModal.public') },
              { id: 'private', icon: 'lock',  label: t('betModal.private') },
            ].map(tab => (
              <button key={tab.id} onClick={() => { setChannelTab(tab.id); setChannelSearch('') }}
                style={{ flex: 1, padding: '7px', borderRadius: 'var(--radius-md)', border: 'none', cursor: 'pointer', fontSize: '12px', fontWeight: 700, fontFamily: 'var(--font-sans)', transition: 'all 0.15s', background: channelTab === tab.id ? 'var(--color-primary)' : 'transparent', color: channelTab === tab.id ? '#010906' : 'var(--color-text-muted)', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '5px' }}>
                <AppIcon name={tab.icon} size={12} />{tab.label}
              </button>
            ))}
          </div>
          ...
          <div style={{ position: 'relative', marginBottom: '8px' }}>
            ...
            <input
              type="text"
              placeholder={t('betModal.searchChannelPlaceholder')}
              value={channelSearch}
            ...
            {(() => {
              ...
              if (list.length === 0) return (
                <div style={{ fontSize: '13px', color: 'var(--color-text-muted)', textAlign: 'center', padding: '14px 0' }}>
                  {channelSearch
                    ? t('betModal.noResults', { q: channelSearch })
                    : channelTab === 'public' ? t('betModal.noPublicChannels') : t('betModal.noPrivateChannels')}
                </div>
              )
      ...
      <AnimatePresence>
        {showCreateChannel ? (
          <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }}
            ...
            <div style={{ fontSize: '15px', fontWeight: 600, marginBottom: '14px' }}>{t('betModal.createChannelTitle')}</div>
            {createError && (
            ...
            <div style={{ marginBottom: '14px' }}>
              ...
              <input autoFocus type="text" placeholder={t('betModal.namePlaceholder')} value={createForm.name}
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '18px', padding: '14px', background: 'var(--color-bg-soft)', borderRadius: 'var(--radius-md)', border: '0.5px solid var(--color-border)', cursor: 'pointer' }}
              ...
              <div>
                <div style={{ fontSize: '13px', fontWeight: 600, display: 'flex', alignItems: 'center', gap: '6px' }}><AppIcon name="lock" size={13} /> {t('betModal.privateChannel')}</div>
                <div style={{ fontSize: '12px', color: 'var(--color-text-muted)', marginTop: '2px' }}>{t('betModal.privateDesc')}</div>
              </div>
            </div>
            <div style={{ display: 'flex', gap: '8px' }}>
              <Button onClick={handleCreateChannel} disabled={creating || !createForm.name.trim()}>{t(creating ? 'betModal.creating' : 'betModal.create')}</Button>
              <Button variant="ghost" onClick={() => { setShowCreateChannel(false); setCreateForm({ name: '', description: '', isPrivate: false }); setCreateError('') }}>{t('common.cancel')}</Button>
            </div>
          </motion.div>
        ) : (
          <motion.button initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <span>+</span>{t(myChannels.length === 0 ? 'betModal.createFirst' : 'betModal.createNew')}
          </motion.button>
        )}
      </AnimatePresence>
    </div>
        ...
        <motion.div className="modal-overlay"
          ...
          <motion.div className="modal"
            ...
            <div className="modal-header">
              <div className="modal-title">{t('betModal.title')}</div>
            ...
            <div style={{ display: 'flex', gap: '4px', background: 'var(--color-bg-soft)', border: '0.5px solid var(--color-border)', borderRadius: 'var(--radius-lg)', padding: '4px', marginBottom: '20px' }}>
              {[
                { id: 'foto',   icon: 'camera',   label: t('betModal.modePhoto') },
                { id: 'manual', icon: 'document', label: t('betModal.modeManual') },
              ].map(m => (
            ...
            {mode === 'manual' && (
              <>
                <div className="form-group-modal">
                  <FormLabel>{t('betModal.eventLabel')}</FormLabel>
                  <Input placeholder={t('betModal.eventPlaceholder')}
                    value={form.event} onChange={e => set('event', stripEmojis(e.target.value))} maxLength={100} />
                </div>

                <div className="form-row-modal">
                  <div>
                    <FormLabel>{t('betModal.sportLabel')}</FormLabel>
                    <select className="input" value={form.sport} onChange={e => set('sport', e.target.value)}>
                      {SPORTS.map(s => <option key={s} value={s}>{s === 'Otros' ? t('betModal.others') : s}</option>)}
                    </select>
                  </div>
                  <div>
                    <FormLabel>{t('betModal.marketLabel')}</FormLabel>
                    <select className="input" value={form.market} onChange={e => set('market', e.target.value)}>
                      {MARKETS.map(m => {
                        const label = m === 'Ambos marcan' ? t('betModal.bothScore') : m === 'Otro' ? t('betModal.other') : m
                ...
                <div className="form-row-modal">
                  <div>
                    <FormLabel>{t('betModal.pickLabel')}</FormLabel>
                    <Input placeholder={t('betModal.pickPlaceholder')}
                      value={form.pick} onChange={e => set('pick', stripEmojis(e.target.value))} maxLength={100} />
                  </div>
                  <div>
                    <FormLabel>{t('betModal.oddsLabel')}</FormLabel>
                    <Input type="number" placeholder={t('betModal.oddsPlaceholder')} step="0.01" min="1.01"
                ...
                <div className="form-row-modal">
                  <div>
                    <FormLabel>{t('betModal.bookieLabel')}</FormLabel>
                    <select className="input" value={form.bookie || ''} onChange={e => set('bookie', e.target.value)}>
                      <option value="">{t('betModal.noBookieOption')}</option>
                      {BOOKIES.map(b => <option key={b}>{b}</option>)}
                    </select>
                  </div>
                  <div>
                    <FormLabel>{t('betModal.dateLabel')}</FormLabel>
                ...
                <div className="form-group-modal">
                  <FormLabel>{t('betModal.stakeLabel')}</FormLabel>
                  <div className="stake-display">{form.stake}</div>
                  <div className="stake-sub">{t('betModal.stakeSub')}</div>
                ...
                <div className="form-group-modal">
                  <FormLabel>{t('betModal.analysisLabel')}</FormLabel>
                  <textarea className="input" rows="3" style={{ resize: 'vertical' }}
                    placeholder={t('betModal.analysisPlaceholder')}
            ...
            {mode === 'foto' && (
              <>
                {/* Upload zone */}
                <div className="form-group-modal">
                  <FormLabel>{t('betModal.ticketLabel')}</FormLabel>
                  ...
                  {form.betImageUrl ? (
                    ...
                    <div
                      ...
                      {uploading ? (
                        <div style={{ fontSize: '13px', color: 'var(--color-text-muted)', display: 'flex', alignItems: 'center', gap: '6px' }}><AppIcon name="loading" size={14} /> {t('betModal.uploading')}</div>
                      ) : (
                        <>
                          <div style={{ marginBottom: '8px', color: 'var(--color-text-muted)' }}><AppIcon name="camera" size={36} /></div>
                          <div style={{ fontSize: '14px', fontWeight: 600, marginBottom: '4px' }}>{t('betModal.uploadTitle')}</div>
                          <div style={{ fontSize: '12px', color: 'var(--color-text-muted)' }}>{t('betModal.uploadDesc')}</div>
                        </>
                      )}
                    </div>
                  )}
                ...
                <div className="form-row-modal">
                  ...
                  <div>
                    <FormLabel>{t('betModal.dateLabelRequired')}</FormLabel>
                    <Input type="datetime-local" value={form.date}
                      min={getMinDateTime()} onChange={e => set('date', e.target.value)} />
                  </div>
                </div>
                ...
                <div className="form-group-modal">
                  <FormLabel>{t('betModal.bookieOptional')}</FormLabel>
                  <select className="input" value={form.bookie || ''} onChange={e => set('bookie', e.target.value)}>
                    <option value="">{t('betModal.noBookieOption')}</option>
                    {BOOKIES.map(b => <option key={b}>{b}</option>)}
                  </select>
            ...
            {preselectedChannelId && (
              <div style={{ fontSize: '13px', color: 'var(--color-text-muted)', marginBottom: '18px', padding: '10px 12px', background: 'var(--color-primary-light)', borderRadius: 'var(--radius-md)', border: '0.5px solid var(--color-primary-border)' }}>
                <span style={{ display: 'flex', alignItems: 'center', gap: '6px' }}><AppIcon name="canales" size={13} /> {t('betModal.channelNote')}</span>
              </div>
            )}
            ...
            <label onClick={() => setConfirmed(v => !v)}
              ...
              <span style={{ fontSize: '12px', color: confirmed ? 'var(--color-text)' : 'var(--color-text-muted)', lineHeight: 1.6 }}>
                {t('betModal.confirmPre')}<strong>{t('betModal.confirmBold')}</strong>{t('betModal.confirmPost')}
              </span>
            </label>

            <Button full onClick={() => onSubmit(preselectedChannelId)} disabled={!canSubmit}>
              <AppIcon name="send" size={14} style={{ marginRight: 6 }} /> {t('betModal.submit')}
            </Button>
          </motion.div>
        </motion.div>
      )}
