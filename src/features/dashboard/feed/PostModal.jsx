      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '5px', marginBottom: '2px', flexWrap: 'wrap' }}>
          ...
          <span style={{ fontSize: '10px', color: 'var(--color-text-muted)' }}>
            {timeAgo(comment.created_at, t)} · {fmtTime(comment.created_at)}
          </span>
          {creatorLiked && !isReply && (
            <span style={{ fontSize: '10px', background: 'var(--color-primary-light)', color: 'var(--color-primary)', border: '0.5px solid var(--color-primary-border)', borderRadius: 'var(--radius-full)', padding: '1px 7px', fontWeight: 700, display: 'inline-flex', alignItems: 'center', gap: '2px', lineHeight: 1.4 }}>
              <AppIcon name="heart" size={10} color="var(--color-primary)" /> {t('postModal.creator')}
            </span>
          )}
        </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '2px', flexWrap: 'wrap' }}>
          ...
          {!isReply && onReply && (
            <button onClick={onReply}
              ...
              <AppIcon name="arrowOut" size={11} /> {t('postModal.reply')}
            </button>
          )}
          {canPin && onPin && (
            <button onClick={onPin}
              ...
              <AppIcon name="pin" size={11} />{isPinned ? t('postModal.unpin') : ''}
            </button>
          )}
          {canDelete && onDelete && (
    ...
    <>
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
        ...
        <motion.div initial={{ opacity: 0, scale: 0.96, y: 16 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95 }}
          ...
          {loading ? (
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '60px', color: 'var(--color-text-muted)', fontSize: '13px', gap: '6px' }}><AppIcon name="loading" size={14} /> {t('contact.loading')}</div>
          ) : !data ? (
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '50px 30px', color: 'var(--color-text-muted)', fontSize: '13px', textAlign: 'center', gap: '12px' }}>
              ...
              <div>{t('postModal.loadError')}</div>
              <button onClick={onClose} style={{ marginTop: '8px', padding: '8px 18px', borderRadius: 'var(--radius-md)', border: '0.5px solid var(--color-border)', background: 'var(--color-bg-soft)', color: 'var(--color-text)', cursor: 'pointer', fontSize: '12px', fontWeight: 600, fontFamily: 'var(--font-sans)' }}>{t('common.close')}</button>
            </div>
          ) : (
            <>
              {/* HEADER */}
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '14px 16px', borderBottom: '0.5px solid var(--color-border)', flexShrink: 0 }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: '11px', color: 'var(--color-text-muted)', display: 'flex', gap: '6px', alignItems: 'center', flexWrap: 'wrap' }}>
                    <span>{timeAgo(data.message.created_at, t)} · {fmtTime(data.message.created_at)}</span>
                    {data.channel && (
                      <>
                        <span>·</span>
                        {data.channelDeleted
                          ? <span style={{ fontStyle: 'italic', opacity: 0.7 }}>{t('profile.channelDeleted')}</span>
                    ...
                    {data.wasPrivate && (
                      <>
                        <span>·</span>
                        <span style={{ fontSize: '10px', background: 'var(--color-bg-soft)', color: 'var(--color-text-muted)', padding: '1px 7px', borderRadius: 'var(--radius-full)', border: '0.5px solid var(--color-border)', fontWeight: 600, display: 'inline-flex', alignItems: 'center', gap: '3px' }}><AppIcon name="lock" size={10} /> {t('betModal.private')}</span>
                      </>
                    )}
                  </div>
                </div>
                      ...
                      <>
                        ...
                        <motion.div initial={{ opacity: 0, scale: 0.95, y: -4 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95 }}
                          ...
                          <button onClick={() => { setReported(true); setShowMenu(false); setTimeout(() => setReported(false), 3000) }}
                            ...
                            <AppIcon name={reported ? 'check' : 'flag'} size={13} /><span>{reported ? t('postModal.reported') : t('social.report')}</span>
                          </button>
                          {adminMode && data?.bet && (
                            <>
                    ...
                    {data.restricted ? (
                      <>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '8px', marginBottom: '12px' }}>
                          <div style={{ fontWeight: 700, fontSize: '14px', lineHeight: 1.3, flex: 1, color: 'var(--color-text-muted)', fontStyle: 'italic', display: 'flex', alignItems: 'center', gap: '5px' }}><AppIcon name="lock" size={14} /> {t('postModal.privatePick')}</div>
                          <span style={{ flexShrink: 0, padding: '3px 10px', borderRadius: 'var(--radius-full)', fontSize: '11px', fontWeight: 700, background: cfg.bg, color: cfg.color, border: `0.5px solid ${cfg.border}` }}>{t('historial.status.' + (data?.bet?.status ?? 'pending'))}</span>
                        </div>
                        <div style={{ display: 'flex', gap: '24px' }}>
                          <div>
                            <div style={{ fontSize: '9px', color: 'var(--color-text-muted)', textTransform: 'uppercase', letterSpacing: '0.8px', marginBottom: '2px' }}>{t('betModal.oddsLabel')}</div>
                            <div style={{ fontWeight: 700, fontSize: '16px' }}>{parseFloat(data.bet.odds || 0).toFixed(2)}</div>
                          </div>
                          <div>
                            <div style={{ fontSize: '9px', color: 'var(--color-text-muted)', textTransform: 'uppercase', letterSpacing: '0.8px', marginBottom: '2px' }}>{t('betModal.stakeLabel')}</div>
                            <div style={{ fontWeight: 700, fontSize: '16px' }}>{data.bet.stake}</div>
                          </div>
                        </div>
                        <div style={{ marginTop: '14px', fontSize: '11px', color: 'var(--color-text-muted)', lineHeight: 1.5 }}>{t('postModal.joinToSee')}</div>
                      </>
                    ) : (
                      <>
                        ...
                        {!data.bet.imageUrl && (
                          <div style={{ display: 'flex', gap: '5px', flexWrap: 'wrap', marginBottom: '12px' }}>
                            {[data.bet.sport, data.bet.market].filter(Boolean).map((tag, i) => (
                              <span key={i} style={{ padding: '2px 8px', borderRadius: 'var(--radius-sm)', background: 'var(--color-bg)', border: '0.5px solid var(--color-border)', fontSize: '10px', color: 'var(--color-text-muted)', fontWeight: 500 }}>{tag}</span>
                            ))}
                        ...
                        <div style={{ display: 'flex', gap: '24px', flexWrap: 'wrap' }}>
                          ...
                          {!data.bet.imageUrl && (
                            <div>
                              <div style={{ fontSize: '9px', color: 'var(--color-text-muted)', textTransform: 'uppercase', letterSpacing: '0.8px', marginBottom: '2px' }}>{t('postModal.dateLabel')}</div>
                              <div style={{ fontSize: '11px', color: 'var(--color-text-muted)', marginTop: '4px' }}>
                                {data.bet.date ? new Date(data.bet.date).toLocaleString(undefined, { day: '2-digit', month: '2-digit', year: 'numeric', hour: '2-digit', minute: '2-digit' }) : '—'}
                              </div>
                            </div>
                          )}
                          {data.bet.bookie && (
                            <div>
                              <div style={{ fontSize: '9px', color: 'var(--color-text-muted)', textTransform: 'uppercase', letterSpacing: '0.8px', marginBottom: '2px' }}>{t('betModal.bookieLabel')}</div>
                ...
                <div style={{ flex: 1, padding: '12px 16px', display: 'flex', flexDirection: 'column', gap: '14px', minHeight: '60px' }}>
                  {data.restricted ? (
                    <div style={{ textAlign: 'center', color: 'var(--color-text-muted)', fontSize: '12px', padding: '12px 0', fontStyle: 'italic' }}>
                      {t('postModal.restrictedComments')}
                    </div>
                  ) : (
                    <>
                      {/* COMENTARI FIXAT */}
                      {pinnedComment && (
                        <div style={{ background: 'var(--color-primary-light)', border: '0.5px solid var(--color-primary-border)', borderRadius: 'var(--radius-md)', padding: '10px 12px' }}>
                          <div style={{ fontSize: '10px', fontWeight: 700, color: 'var(--color-primary)', marginBottom: '8px', display: 'flex', alignItems: 'center', gap: '4px' }}>
                            <AppIcon name="pin" size={10} /> {t('postModal.pinnedComment')}
                          </div>
                          <CommentItem
                                ...
                                {isReplyingToThis && (
                                  <div style={{ display: 'flex', gap: '6px', alignItems: 'center' }}>
                                    <input
                                      ...
                                      placeholder={t('postModal.replyPlaceholder', { username: replyingTo?.username || t('postModal.defaultUser') })}
                                      maxLength={500}
        ...
        {showEditResult && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ opacity: 0, y: 20, scale: 0.96 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 20, scale: 0.96 }}
              ...
              <label style={{ display: 'block', fontSize: '12px', fontWeight: 600, color: 'var(--color-text-muted)', textTransform: 'uppercase', letterSpacing: '0.8px', marginBottom: '6px' }}>{t('bets.statusLabel')}</label>
              <select value={editStatus} onChange={e => setEditStatus(e.target.value)}
                ...
                <option value="pending">{t('bets.status.pending')}</option>
                <option value="won">{t('bets.status.won')}</option>
                <option value="lost">{t('bets.status.lost')}</option>
                <option value="void">{t('bets.status.voidFull')}</option>
              </select>
